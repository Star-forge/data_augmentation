...
def modify_image(im, isNoiseFill):
    filled = ""
...
if __name__ == "__main__":
    ...
    # Фильтрация списка, по расширениям '.jpg' или '.gif'
    images = filter(lambda x: x.endswith('.jpg' or '.gif'), files)

    #
    # растягивание и сжатие по осям X и Y на определенное расстояние,
    # сдвиги,
    # введение на фото бликов и линий,
    # дефокус.
    ...
    for image_name in images:
        print(image_name)
        im = get_image_from_path(directory, image_name)
        modify_image(im, noise)
        blurred_image = im.filter(ImageFilter.GaussianBlur(radius=5))
